...
# Helper to obtain real User model instance
...
def get_dashboard_stats(
    ...
    total_livestock = db.query(models.Livestock).filter(models.Livestock.owner_id == db_user.id).count()
    total_crops = db.query(models.Crop).filter(models.Crop.owner_id == db_user.id).count()
    inventory_items = db.query(models.InventoryItem).filter(models.InventoryItem.owner_id == db_user.id).count()
    active_workers = db.query(models.Worker).filter(models.Worker.owner_id == db_user.id).count()
